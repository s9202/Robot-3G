...
//-------------------------------------------------------- Klawiatura
	...
	$("#mapa").mousemove(function(event) {
		...
		console.log(msg);
		if (setBlock == true) {
			drawBlock2(event.pageX, event.pageY);
		}
		if (delBlock == true) {
			drawCross(event.pageX, event.pageY);
		}
		if (setRobot == true) {
			drawRobot2(event.pageX, event.pageY);
		}
...
//---------------------------------------------------------------------funkcje pomocnicze
	function setCharAt(str, index, chr){// funkcja do zamiany znaku stringa
